...
| 高校英語文法：動詞 | **進行中** | 最初の11トピック・正規動画22本を登録・検証済み |
...
| 動画ID | タイトル | 状態 |
| --- | --- | --- |
| `h91S1Cm0o4w` | 【高校 英語】 suit / match / go with / fit① （5分） | 登録・表示・検証済み |
| `KY5mVM_4IBU` | 【高校 英語】 suit / match / go with / fit② （8分） | 登録・DB検証済み |

## 3. 直近の品質確認結果

直近の似合う系動詞（suit / match / go with / fit）2本は、Try IT公式のポイント・練習・チャレンジ、公開埋め込みID、カタログを照合済みである。suitの人への似合い方、match／go withの物どうしの調和、fitのサイズ・型の適合、matchとgo withの語順を含むノートを登録し、`h91S1Cm0o4w`の視聴ページで、YouTubeプレイヤー、要約、覚えるポイント、復習観点が表示されることを確認した。登録内容はSQLで2本とも必須語句・復習観点を含むことを確認し、2026-08-17時点でVitestは**6ファイル・24テストすべて成功**している。

表示確認には、`webdev_take_screenshot`が一時的にスピナーのままになる場合があった。この場合でも、再起動後にプレビューURLを直接開くと視聴ページとノート表示が確認できた。データ登録が完了しているかは必ずSQLで確認する。

## 4. 次に行う作業

高校英語文法「動詞」の次の公式トピックは、**使役動詞（make / have / let）**である。以後も、Try IT公式の動詞一覧にある順番で進める。

続く主なトピックは、使役動詞、知覚動詞、そして各種の動詞構文である。
...
> まずは「動詞」の次トピックである使役動詞（make / have / let）を1バッチ（通常2本）だけ公式根拠から作成し、登録・DB検証・表示確認・Vitest・進捗記録・保存・GitHub反映までを完結させてください。品質基準を落とさず、この繰り返しを全動画まで続けることが最優先です。
