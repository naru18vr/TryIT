...
| 高校英語文法：動詞 | **進行中** | 最初の9トピック・正規動画18本を登録・検証済み |
...
| 動画ID | タイトル | 状態 |
| --- | --- | --- |
| `8P_l3sR0WK4` | 【高校 英語】 forgive / permit / allow① （4分） | 登録・表示・検証済み |
| `Q6gD9jcDWCM` | 【高校 英語】 forgive / permit / allow② （6分） | 登録・DB検証済み |

## 3. 直近の品質確認結果

直近の許す系動詞（forgive / permit / allow）2本は、Try IT公式のポイント・練習・チャレンジ、公開埋め込みID、カタログを照合済みである。forgive 人 for 物とpermit／allow 人 to Vの区別を含むノートを登録し、`8P_l3sR0WK4`の視聴ページで、YouTubeプレイヤー、要約、覚えるポイント、復習観点が表示されることを確認した。登録内容はSQLで2本とも必須語句・復習観点を含むことを確認し、2026-08-17時点でVitestは**6ファイル・24テストすべて成功**している。

表示確認には、`webdev_take_screenshot`が一時的にスピナーのままになる場合があった。この場合でも、再起動後にプレビューURLを直接開くと視聴ページとノート表示が確認できた。データ登録が完了しているかは必ずSQLで確認する。

## 4. 次に行う作業

高校英語文法「動詞」の次の公式トピックは、**疑う系動詞（doubt / suspect）の使い分け**である。以後も、Try IT公式の動詞一覧にある順番で進める。

続く主なトピックは、doubt/suspect、suit/match/go with/fit、使役動詞、知覚動詞、そして各種の動詞構文である。
...
> まずは「動詞」の次トピックである疑う系動詞（doubt / suspect）を1バッチ（通常2本）だけ公式根拠から作成し、登録・DB検証・表示確認・Vitest・進捗記録・保存・GitHub反映までを完結させてください。品質基準を落とさず、この繰り返しを全動画まで続けることが最優先です。
