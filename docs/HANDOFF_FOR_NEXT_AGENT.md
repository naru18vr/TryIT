...
| 高校英語文法：動詞 | **進行中** | 最初の15トピック・正規動画30本を登録・検証済み |
...
| 動画ID | タイトル | 状態 |
| --- | --- | --- |
| `Xjxl-kPyF2g` | 【高校 英語】 prevent / distinguish など① （7分） | 登録・表示・検証済み |
| `m9mVD_HKfsk` | 【高校 英語】 prevent / distinguish など② （8分） | 登録・DB検証済み |

## 3. 直近の品質確認結果

直近のprevent A from doing型・distinguish A from B型動詞2本は、Try IT公式のポイント・練習・チャレンジ、公開埋め込みID、カタログを照合済みである。prevent A from Ving、distinguish／tell A from Bの語順、fromの後ろの動名詞、無生物主語を自然に読む方法を含むノートを登録し、`Xjxl-kPyF2g`の視聴ページで、YouTubeプレイヤー、要約、覚えるポイント、復習観点が表示されることを確認した。登録内容はSQLで2本とも必須語句・復習観点を含むことを確認し、2026-08-17時点でVitestは**6ファイル・24テストすべて成功**している。

表示確認には、`webdev_take_screenshot`が一時的にスピナーのままになる場合があった。この場合でも、再起動後にプレビューURLを直接開くと視聴ページとノート表示が確認できた。データ登録が完了しているかは必ずSQLで確認する。

## 4. 次に行う作業

高校英語文法「動詞」の次の公式トピックは、**regard A as Bタイプの動詞・talk A into doingタイプの動詞**である。以後も、Try IT公式の動詞一覧にある順番で進める。

続く主なトピックは、regard A as Bタイプ・talk A into doingタイプ、そして各種の動詞構文である。
...
> まずは「動詞」の次トピックであるregard A as Bタイプの動詞・talk A into doingタイプの動詞を1バッチ（通常2本）だけ公式根拠から作成し、登録・DB検証・表示確認・Vitest・進捗記録・保存・GitHub反映までを完結させてください。品質基準を落とさず、この繰り返しを全動画まで続けることが最優先です。
