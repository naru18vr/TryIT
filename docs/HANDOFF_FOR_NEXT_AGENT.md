...
| 高校英語文法：動詞 | **進行中** | 最初の7トピック・正規動画14本を登録・検証済み |
...
| 動画ID | タイトル | 状態 |
| --- | --- | --- |
| `NaVgkW7Uhgs` | 【高校 英語】 自動詞と間違えやすい他動詞① （6分） | 登録・表示・検証済み |
| `ZZhNESfJ0B4` | 【高校 英語】 自動詞と間違えやすい他動詞② （5分） | 登録・DB検証済み |

## 3. 直近の品質確認結果

直近の自動詞と間違えやすい他動詞2本は、Try IT公式のポイント・練習・チャレンジ、公開埋め込みID、カタログを照合済みである。ノートの登録後、`NaVgkW7Uhgs`の視聴ページで、YouTubeプレイヤー、要約、覚えるポイント、復習観点が表示されることを確認した。2026-08-17時点で、Vitestは**6ファイル・24テストすべて成功**している。

表示確認には、`webdev_take_screenshot`が一時的にスピナーのままになる場合があった。この場合でも、再起動後にプレビューURLを直接開くと視聴ページとノート表示が確認できた。データ登録が完了しているかは必ずSQLで確認する。

## 4. 次に行う作業

高校英語文法「動詞」の次の公式トピックは、**貸す・借りる系動詞（borrow / lend / rent / use）の使い分け**である。以後も、Try IT公式の動詞一覧にある順番で進める。

続く主なトピックは、注意すべき自動詞、注意すべき他動詞、tell/say/speak/talk、borrow/lend/rent/use、forgive/permit/allow、doubt/suspect、suit/match/go with/fit、使役動詞、知覚動詞、そして各種の動詞構文である。
...
この文書を含む直近変更は、停止前にGitHubへ反映する予定である。引き継ぎ先は作業開始時に`git status`と最新コミットを確認し、未コミット差分がないことを確認してから次の単元へ進む。

> まずは「動詞」の次トピックを1バッチ（通常2本）だけ公式根拠から作成し、登録・DB検証・表示確認・Vitest・進捗記録・保存までを完結させてください。品質基準を落とさず、この繰り返しを全動画まで続けることが最優先です。
