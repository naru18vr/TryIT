...
| 高校英語文法：動詞 | **進行中** | 最初の16トピック・正規動画32本を登録・検証済み |
...
| 動画ID | タイトル | 状態 |
| --- | --- | --- |
| `GmojO1ltphg` | 【高校 英語】 regard A as B など① （6分） | 登録・表示・検証済み |
| `jxpuBklCG7Y` | 【高校 英語】 regard A as B など② （7分） | 登録・DB検証済み |

## 3. 直近の品質確認結果

直近のregard A as B型・talk A into doing型動詞2本は、Try IT公式のポイント・練習・チャレンジ、公開埋め込みID、カタログを照合済みである。regard／refer A as B、talk A into Vingの語順、asの後ろのB、intoの後ろの動名詞を含むノートを登録し、`GmojO1ltphg`の視聴ページで、YouTubeプレイヤー、要約、覚えるポイント、復習観点が表示されることを確認した。登録内容はSQLで2本とも必須語句・復習観点を含むことを確認し、2026-08-17時点でVitestは**6ファイル・24テストすべて成功**している。

表示確認には、`webdev_take_screenshot`が一時的にスピナーのままになる場合があった。この場合でも、再起動後にプレビューURLを直接開くと視聴ページとノート表示が確認できた。データ登録が完了しているかは必ずSQLで確認する。

## 4. 次に行う作業

高校英語文法「動詞」の次の公式トピックは、**blame A for Bタイプの動詞・provide A with Bタイプの動詞**である。以後も、Try IT公式の動詞一覧にある順番で進める。

続く主なトピックは、blame A for Bタイプ・provide A with Bタイプであり、これで動詞単元の公式トピックを完了する。
...
> まずは「動詞」の次トピックであるblame A for Bタイプの動詞・provide A with Bタイプの動詞を1バッチ（通常2本）だけ公式根拠から作成し、登録・DB検証・表示確認・Vitest・進捗記録・保存・GitHub反映までを完結させてください。このバッチ後に動詞単元の全トピックを最終監査し、品質基準を落とさず次の単元へ進むことが最優先です。
