...
| 高校英語文法：動詞 | **進行中** | 最初の14トピック・正規動画28本を登録・検証済み |
...
| 動画ID | タイトル | 状態 |
| --- | --- | --- |
| `BTldHg_letM` | 【高校 英語】 rob A of B / remind A of B① （6分） | 登録・表示・検証済み |
| `zaTXOicp8ao` | 【高校 英語】 rob A of B / remind A of B② （6分） | 登録・DB検証済み |

## 3. 直近の品質確認結果

直近のrob A of B型・remind A of B型動詞2本は、Try IT公式のポイント・練習・チャレンジ、公開埋め込みID、カタログを照合済みである。rob／deprive A of B、remind／inform A of Bの語順、A・Bの役割、奪う型と結び付ける型の違いを含むノートを登録し、`BTldHg_letM`の視聴ページで、YouTubeプレイヤー、要約、覚えるポイント、復習観点が表示されることを確認した。登録内容はSQLで2本とも必須語句・復習観点を含むことを確認し、2026-08-17時点でVitestは**6ファイル・24テストすべて成功**している。

表示確認には、`webdev_take_screenshot`が一時的にスピナーのままになる場合があった。この場合でも、再起動後にプレビューURLを直接開くと視聴ページとノート表示が確認できた。データ登録が完了しているかは必ずSQLで確認する。

## 4. 次に行う作業

高校英語文法「動詞」の次の公式トピックは、**prevent A from doingタイプの動詞・distinguish A from Bタイプの動詞**である。以後も、Try IT公式の動詞一覧にある順番で進める。

続く主なトピックは、prevent A from doingタイプ・distinguish A from Bタイプ、そして各種の動詞構文である。
...
> まずは「動詞」の次トピックであるprevent A from doingタイプの動詞・distinguish A from Bタイプの動詞を1バッチ（通常2本）だけ公式根拠から作成し、登録・DB検証・表示確認・Vitest・進捗記録・保存・GitHub反映までを完結させてください。品質基準を落とさず、この繰り返しを全動画まで続けることが最優先です。
