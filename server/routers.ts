...
import {
  TRYIT_CATALOG,
  TRYIT_GRADES,
  TRYIT_SUBJECTS_BY_GRADE,
  TRYIT_UNITS_BY_GRADE_AND_SUBJECT,
} from "./data/tryitCatalog";
...
export const appRouter = router({
  ...
  catalog: router({
    filters: publicProcedure.input(catalogFiltersInput).query(({ input }) => {
      ...
      return {
        ...
        units:
          grade && subject
            ? (TRYIT_UNITS_BY_GRADE_AND_SUBJECT[`${grade}::${subject}`] ?? [])
            : [],
    ...
    list: publicProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const matching = TRYIT_CATALOG.filter(item => {
          ...
          const matchesUnit =
            !input.unit || input.unit === "すべて" || item.unit === input.unit;
          const haystack =
            `${item.title} ${item.grade} ${item.subject} ${item.unit}`.toLocaleLowerCase(
              "ja-JP"
            );
          return (
            matchesGrade &&
        ...
        const watchedIds = ctx.user
          ? new Set(
              await db.getUserWatchedVideoIds(
                ctx.user.id,
    ...
    get: publicProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const [note, isWatched] = await Promise.all([
          db.getVideoNote(input.videoId),
          ctx.user
            ? db.hasUserWatchedVideo(ctx.user.id, input.videoId)
  ...
  notes: router({
    coverage: publicProcedure.query(async () => {
      const catalogVideoIds = TRYIT_CATALOG.map(video => video.id);
      const completed = await db.getVideoNoteCount(catalogVideoIds);
      const total = catalogVideoIds.length;
      return {
        completed,
        total,
    ...
    myProgress: protectedProcedure.query(async ({ ctx }) => {
      ...
      return {
        ...
        progressPercentage: totalVideos
          ? Math.round((watchedCount / totalVideos) * 1000) / 10
          : 0,
