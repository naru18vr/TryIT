...
const state = vi.hoisted(() => ({
  ...
  filters: { subjects: ["高校数学", "世界史"], unitsBySubject: { 高校数学: ["二次関数"], 世界史: ["昭和時代"] }, totalVideos: 2 },
  catalog: {
    total: 1,
    page: 1,
    pageSize: 12,
  ...
  catalogInput: null as { subject?: string } | null,
  markWatched: vi.fn(),
  startLogin: vi.fn(),
}));
...
vi.mock("@/components/YouTubePlayer", () => ({
  YouTubePlayer: ({ onPlaybackStarted }: { onPlaybackStarted: () => void }) => <button onClick={onPlaybackStarted}>再生テスト</button>,
}));
vi.mock("@/lib/trpc", () => ({
  trpc: {
    useUtils: () => ({
      catalog: { get: { invalidate: vi.fn() }, list: { invalidate: vi.fn() } },
      learning: { myProgress: { invalidate: vi.fn() } },
    }),
    catalog: {
      filters: { useQuery: () => ({ data: state.filters }) },
      list: { useQuery: (input: { subject?: string }) => { state.catalogInput = input; return { data: state.catalog, isLoading: false }; } },
      get: { useQuery: () => ({ data: state.video, isLoading: false }) },
      markWatched: { useMutation: () => ({ mutate: state.markWatched, isPending: false }) },
    },
    notes: { upsert: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } },
...
describe("learning page rendering", () => {
  ...
  it("renders a watched badge and applies a selected subject to the catalog query", () => {
    render(<Home />);
    expect(screen.getByText("視聴済み")).toBeTruthy();
    fireEvent.click(screen.getByRole("button", { name: "高校数学" }));
    expect(state.catalogInput).toMatchObject({ subject: "高校数学" });
  });
